...
class LearningSwitch(DynamicPolicy):
    ...
    def print_switch_tables(self):
        ...
        for switch in self.fwd_table.keys():
            for mac in self.fwd_table[switch].keys():
                write_forwarding_entry(switch, self.fwd_table[switch][mac],str(mac))
        next_entry()
        finish_log()
    ...
    def learn_route(self, pkt):
        ...
        self.fwd_table[get_switch(pkt)][get_src_mac(pkt)] = get_inport(pkt)
    ...
    def build_policy(self):
        ...
        for switch in self.fwd_table.keys():
            for rule in self.fwd_table[switch].keys():
                if new_policy == None:
                    # First entry, prime the pump
                    new_policy = (match(switch=int(switch), dstmac=(rule))
                                  >> fwd(self.fwd_table[switch][rule]))
                else:
                    new_policy += (match(switch=int(switch), dstmac=(rule))
                                   >> fwd(self.fwd_table[switch][rule]))
                if not_flood_pkts == None:
                    not_flood_pkts = (match(switch=int(switch), dstmac=(rule)))
                else:
                    not_flood_pkts |= (match(switch=int(switch), dstmac=(rule)))
